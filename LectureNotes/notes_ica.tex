...
The corresponding linear transformation \(T:\mathbb{R}^2 \to \mathbb{R}^2\) can be represented as a matrix. By applying to the basic elements \(1 = (1,0), i= (0,1)\) then 
\(T(1) = a_1 + i a_2\), and \(T(i) = -a_2, i a_1\), then the matrix representation of \(T\) is
...
\label{sec:org3689351}

\begin{thm}
Let \(\Omega\subseteq \mathbb{C}\) an open set \(f:\mathbb{C} \to \mathbb{C}\), and \(z_0= x_0 + iy_0 \in \Omega\). Write \(f(z) = u(x,y) + i v(x,y)\), where \(u,v:\Omega \to \mathbb{R}\).
...
\begin{proof}
(\(\Rightarrow\)) Suppose \(f\) is complex differentiable at \(z_0\). Then by definition
...
Now writing the last expression as mapping between \(\mathbb{R}^2\), i.e, \(z= x + i y = (x, y)\), \(z_0= x_0 + i y_0 = (x_0, y_0)\), and using the matrix representatinon of \(f'(z_0) = a_1 + i a_2\), we can rewrite the last equaiton as follows
...
\lim_{z\to z_0} \frac{\left| f(x,y) - f(x_0,y_0)  - T(x - x_0, y - y_0)\right|}{\left| (x - x_0, y - y_0)\right| \]
\end{proof}


\textbf{Remark}. A usual notation is for the Cauchy-Riemann equations is \(u_x = v_y\) and \(u_y = -v_x\).\\
Note that if a function is complex differentiable, then their corresponding functions are \(C^1\).
...
\[\frac{ x^5  - 10 x^3 y^2 + 5 x y^4}{(x^2 + y^2) ^2} + i \frac{5 x^4 y -  10 x^2 y^3  +  y^5}{(x^2 + y^2)^2} \]
\end{document}